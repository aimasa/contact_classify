...
class CRF(nn.Module):
    def __init__(self, label_size, use_cuda, tag_to_ix):
        super().__init__()
    ...
    def _init_weight(self):
        ...
        self.transitions.data[self.tag_to_ix[START_TAG], :].fill_(-10000.)
        self.transitions.data[:, self.tag_to_ix[STOP_TAG]].fill_(-10000.)

    def _score_sentence(self, input, tags):
        bsz, sent_len, l_size = input.size()
        score = self.torch.FloatTensor(bsz).fill_(0.)
        s_score = self.torch.LongTensor([[self.tag_to_ix[START_TAG]]] * bsz)
        ...
        bsz_t = gather_index(temp.transpose(0, 1),
                             (self.torch.LongTensor([self.tag_to_ix[STOP_TAG]] * bsz)))
        return score + bsz_t
    ...
    def forward(self, input):
        ...
        init_alphas[:, self.tag_to_ix[START_TAG]].fill_(0.)
        ...
        forward_var = forward_var + self.transitions[self.tag_to_ix[STOP_TAG]].view(
            1, -1)
    ...
    def viterbi_decode(self, input):
        ...
        init_vvars = self.torch.FloatTensor(
            bsz, self.label_size).fill_(-10000.)
        init_vvars[:, self.tag_to_ix[START_TAG]].fill_(0.)
...
class Model(nn.Module):
    def __init__(self, word_size, word_ebd_dim, lstm_hsz, lstm_layers, dropout, batch_size, label_size, tag_to_ix, use_cuda = True):
        super().__init__()
        ...
        self.crf = CRF(self.label_size, self.use_cuda, self.tag_to_ix)
        self._init_weights()
