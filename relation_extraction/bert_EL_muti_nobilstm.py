...
from relation_extraction.pre import optimization, tokenization
import tensorflow as tf
import numpy as np
from relation_extraction import HeadSelectionScores_noBilstm
flags = tf.flags
import matplotlib.pyplot as plt
FLAGS = flags.FLAGS
from relation_extraction import txt_segmented

# os.environ["CUDA_VISIBLE_DEVICES"]="-1"
## Required parameters
flags.DEFINE_string(
    "data_dir", "D:/data/test/test",
    "The input data dir. Should contain the .tsv files (or other data files) "
    "for the task.")

flags.DEFINE_string(
    "bert_config_file", "F:/phython workspace/deal_contact/bert/bert_config.json",
    "The config json file corresponding to the pre-trained BERT model. "
    "This specifies the model architecture.")

flags.DEFINE_string("task_name", "semeval", "The name of the task to train.")

flags.DEFINE_string("vocab_file", "F:/phython workspace/deal_contact/bert/vocab.txt",
                    "The vocabulary file that the BERT model was trained on.")

flags.DEFINE_string(
    "output_dir", os.path.join(os.path.abspath(os.path.dirname(__file__)), "tmp/bertEL_no/"),
    "The output directory where the model checkpoints will be written.")

## Other parameters

flags.DEFINE_string(
    "init_checkpoint", "F:/phython workspace/deal_contact/bert/bert_model.ckpt",
...
flags.DEFINE_bool("predict_eval", True, "Whether to run eval on the dev set.")
flags.DEFINE_bool(
    "do_predict", True,
...
class InputExample(object):
    ...
    def __init__(self, guid, text_a, text_b, locations, labels, num_relations, joins_id = None, scoringMatrix = None, entity_T = None, path = None):
...
class SemEvalProcessor(DataProcessor):
    ...
    def get_train_examples(self, data_dir):
        """See base class."""
        return self._create_examples(os.path.join(data_dir, "train"), "train", deal_type="full")

    def get_dev_examples(self, data_dir):
        """See base class."""
        return self._create_examples(os.path.join(data_dir, "dev"), "dev", deal_type="full")

    def get_test_examples(self, data_dir):
        """See base class."""
        return self._create_examples(os.path.join(data_dir, "test"), "test", deal_type="full")

    def get_prediction_examples(self, data_dir):
        """See base class."""
        return self._create_prediction_examples(os.path.join(data_dir, "prediction"), "prediction", deal_type="full")

    def get_labels(self, data_dir):
        """See base class."""
        label_list = []
        filein = open(os.path.join(data_dir, "relation_label/label.txt"))
        for line in filein:
            label = line.strip()
    ...
    #     #     #     #     return examples
    def _create_examples(self, data_dir, set_type, deal_type = "normal", type = "train"):
        """Creates examples for the training and dev sets."""
        # examples = []
        #
        ...
        #         InputExample(guid=guid, text_a=text_a, text_b=None,
        #                      locations=locations, labels=labels, num_relations=num_relations))
        examples = txt_segmented.gain_format_data(set_type, data_dir, deal_type)

        return examples

    def _create_prediction_examples(self, data_dir, set_type, deal_type = "normal"):
        """Creates examples for the training and dev sets."""
        return txt_segmented.gain_format_prediction_data(set_type, data_dir, deal_type)
...
def convert_single_example(ex_index, example, label_list, max_seq_length,
    ...
    mapping_a = [i for i in range(len(tokens_a))]
    tokens_b = None
    if example.text_b:
...
def create_model(bert_config, is_training, input_ids, input_mask, segment_ids,
    ...
    #大小
    rel_scores = HeadSelectionScores_noBilstm.getHeadSelectionScores(output_layer, hidden_size, 64, num_labels,
                                             dropout_keep_in_prob=1.0)  # [16, 348, 348*6]
    probas = tf.nn.sigmoid(rel_scores)  # 将输出压缩到0~1
    predictedRel = tf.round(probas)  # 四舍五入 转成0或1
    if scoring_matrix_gold is not None:
        ...
        rel_scores = HeadSelectionScores_noBilstm.getHeadSelectionScores(perturb_inputs, hidden_size, 64, num_labels,
