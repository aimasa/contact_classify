from relation_extraction import score_utils
import copy
...
class chunkEvaluator:
    def __init__(self,labels_name,ner_chunk_eval="boundaries_type",rel_chunk_eval="boundaries"):
        # self.nerSegmentationTags=config.dataset_set_bio_tags
        #
        # self.NERset = config.dataset_set_ec_tags
        self.RELset = labels_name #
    ...
    def add(self, pred_batchesREL, true_batchesREL):
        ...
        for batch_idx in range(1):
            ...
            predRel = pred_batchesREL
            trueRel = true_batchesREL

            ptoken_ids, _, plabel_ids, phead_ids, plabel_names = score_utils.transformToInitialInput(
                predRel, self.RELset)

            _, _, tlabel_ids, thead_ids, tlabel_names = score_utils.transformToInitialInput(
                trueRel, self.RELset)
            ...
            relTrueList = list(trueRel)  # trueRel#

            # if (len(trueRel)!=len(relTrueList)):
            #    print ("warning")

            relPredList = list(predRel)  # predRel#
            ...
            self.correct_predsREL += len(set(trueRel) & set(predRel))
            self.total_predsREL += len(set(predRel))
            self.total_correctREL += len(set(trueRel))


    def precision(self, pred_batchesREL):
    ...
    #         for label.tsv in self.NERset:
    #             # if label.tsv != "O" :
    #             tpsNER += self.tpsClassesNER[label.tsv]
    #
    #             fnsNER += self.fnsClassesNER[label.tsv]
    #             fpsNER += self.fpsClassesNER[label.tsv]
    ...
    #
    #
    #     for label.tsv in self.RELset:
    #
    #         if label.tsv != "N":
    #             tpsREL += self.tpsClassesREL[label.tsv]
    #
    #             fnsREL += self.fnsClassesREL[label.tsv]
    #             fpsREL += self.fpsClassesREL[label.tsv]
    #
    #
    #
    ...
    def printInfo(self):
        ...
        #     for label.tsv in self.NERset:
        #         # if label.tsv != "O" :
        #         self.tpsNER += self.tpsClassesNER[label.tsv]
        #
        #         self.fnsNER += self.fnsClassesNER[label.tsv]
        #         self.fpsNER += self.fpsClassesNER[label.tsv]
        #
        #         printer.add(label.tsv, self.tpsClassesNER[label.tsv], self.fpsClassesNER[label.tsv], self.fnsClassesNER[label.tsv],
        #                     self.getPrecision(self.tpsClassesNER[label.tsv], self.fpsClassesNER[label.tsv]),
        #                     self.getRecall(self.tpsClassesNER[label.tsv], self.fnsClassesNER[label.tsv]),
        #                     self.getF1(self.tpsClassesNER[label.tsv], self.fpsClassesNER[label.tsv], self.fnsClassesNER[label.tsv]))
        #
        #
        #
        #         # print('%s TP: %d  FP: %d  FN: %d TN: %d precision: %f recall: %f F1: %f' % (label.tsv,self.tpsClasses[label.tsv],self.fpsClasses[label.tsv],self.fnsClasses[label.tsv],self.tnsClasses[label.tsv], self.precision[label.tsv], self.recall[label.tsv], self.f1[label.tsv]))
        #     printer.add("-", "-", "-", "-",
        #                 "-", "-",
        #                 "-")
        ...
        printer.print()
        return self.getPrecision(self.tpsREL, self.fpsREL), self.getRecall(self.tpsREL, self.fnsREL), self.getF1(self.tpsREL, self.fpsREL, self.fnsREL)
...
class relaxedChunkEvaluator:
    ...
    def add(self, pred_batchesNER, true_batchesNER, pred_batchesREL, true_batchesREL,true_batchesBIONER):
        ...
        for batch_idx in range(len(pred_batchesNER)):
            ...
            #print (self.NERset)
            tagsNER = score_utils.getSegmentationDict(self.nerSegmentationTags)#self.
