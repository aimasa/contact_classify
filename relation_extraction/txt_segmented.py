...
import os
from relation_extraction import ann_to_example
from collections import defaultdict
import copy
class InputExample(object):
    """A single training/test example for simple sequence classification."""

    def __init__(self, guid, text_a, text_b, locations, labels, num_relations, entity_T, path):
...
def get_pre_file_path(base_path):
    '''将ann和txt文件分开；来获取路径列表
    :param base_path (str) 基础路径（包括ann和txt的）
    :return anns ann后缀的路径
    :return txt txt后缀的路径'''
    ...
    # anns = [os.path.join(base_path, i) for i in ann_files]
    txts = [os.path.join(base_path, i) for i in txt_files]
    return txts, txt_files

def gain_format_data(type, path, deal_type="normal"):
    '''
    获得格式化的训练数据
    :param path:
    :param deal_type normal 只处理存在关系的实体对，full 对不存在关系的实体对赋值关系N
    :return:
    '''
    ...
    if deal_type == "normal":
        example, max_label_num = format_relation_and_entity(type, txt_segments, all_labels, entity_T, ann_paths)
    else:
        example, _ = full_format_relation_and_entity(type, txt_segments, all_labels, entity_T, ann_paths)
    normal_util.shuffle_(example)
    return example

def gain_format_prediction_data(type, path, deal_type="normal"):
    '''
     获得格式化的预测数据
     :param path:
     :param deal_type normal 只处理存在关系的实体对，full 对不存在关系的实体对赋值关系N
    ...
    entity_T_copy = copy.deepcopy(entity_T)
    all_labels = updata_entity_T(entity_T, label_segments)
    if deal_type == "normal":
        example, max_label_num, num_sentences = format_prediction_relation_and_entity(type, txt_segments, all_labels, entity_T, ann_paths)
    else:
        example, _, num_sentences = full_format_prediction_relation_and_entity(type, txt_segments, all_labels, entity_T, ann_paths)
    normal_util.shuffle_(example)
    return example, entity_T_copy, num_sentences
...
def format_relation_and_entity(type, txt_segments, all_labels, entity_T, paths = None):
    '''
    ...
    for index in range(len(txt_segments)):
        for next_index in range(len(txt_segments[index])):
            ...
            example.append(InputExample(guid=guid, text_a="".join(txt_segments[index][next_index]), text_b=None,
                             locations=entity_local, labels=labels_name, num_relations=len(labels_name), entity_T=index, path=paths[index]))
            num += 1
    return example, max_label_name

def format_prediction_relation_and_entity(type, txt_segments, all_labels, entity_T, paths = None):
    '''
    将实体和关系通过entity dic格式化成example
    :param txt_segment: 句子list
    :param all_labels: 每个句子中的所有label
    ...
    return example, max_label_name, all_num_sentences


def full_format_relation_and_entity(type, txt_segments, all_labels, entity_T, ann_paths=None):
    '''
    将实体和关系通过entity dic格式化成example
    :param txt_segment: 句子list
    :param all_labels: 每个句子中的所有label
    ...
    for index in range(len(txt_segments)):
        for next_index in range(len(txt_segments[index])):
            guid = "%s-%s"%(type, num)
            labels_name, entity_local = gain_full_label_and_local(all_labels[index][next_index], entity_T[index])
            if len(labels_name) <= 0:
            ...
            example.append(InputExample(guid=guid, text_a="".join(txt_segments[index][next_index]), text_b=None,
                             locations=entity_local, labels=labels_name, num_relations=len(labels_name),entity_T=index, path = ann_paths[index]))
            num += 1
    return example, max_label_name

def full_format_prediction_relation_and_entity(type, txt_segments, all_labels, entity_T, ann_paths=None):
    '''
    预测中，将实体和关系通过entity dic格式化成example
    :param txt_segment: 句子list
    :param all_labels: 每个句子中的所有label
    :param entity_T: 实体dic
    :param num_relations: 关系类别数量
    ...
    for index in range(len(txt_segments)):
        ...
        for next_index in range(len(txt_segments[index])):
            ...
            if len(labels_name) <= 0:
                num += 1
                num_sentence = len(txt_segments[index][next_index]) + 2 + (num_sentences[next_index] if next_index > 0 else 0)
                continue
            max_label_name = max(max_label_name, len(labels_name))
...
def gain_full_label_and_local(all_label, entity_T):
    '''
    获得当前所有labels对应的关系和labels组成关系的第二个实体的local
    :param all_labels: 当前句子中存在的所有label
    :param entity_T: 实体dic
    ...
    for label_no in all_label:
        T_dic = entity_T[label_no]
        entity_list = []
        entity_list.append(label_no)
        if  "relation_info" in T_dic:
            relation_dic = T_dic["relation_info"]
            for key in relation_dic:
                labels_name.append(relation_dic[key])
                T2_dic = entity_T[key]
                ...
                entity_local.append((updata_local(T_dic["local"]), updata_local(T2_dic["local"])))
        for label2_no in all_label:
            if label2_no in entity_list:
                continue
            labels_name.append("N")
            entity_local.append((updata_local(T_dic["local"]), updata_local(entity_T[label2_no]["local"])))
    return labels_name, entity_local
