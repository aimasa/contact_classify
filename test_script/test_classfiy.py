...
def pro_data(texts_path):
    ...
    content = pro.load_data_docx(texts_path)
    contents += [content]
...
def get_array(texts_content):
    ...
    vocab_processor = learn.preprocessing.VocabularyProcessor.restore("F:/phython workspace/deal_contact_v2/contact_classify/vocab")
...
def test(data_array):
    ...
    with tf.Session() as sess:
        saver = tf.train.import_meta_graph('F:/phython workspace/deal_contact/contact_classify/runs/' + normal_param.model_name + '/checkpoint/model-900.meta')
